...
vorpal
  .command('create plugin <name>', 'Create basic plugin template')
  .action(function(args, cb) {
    ...
    const pack = Fs.readFileSync(
      Path.join(rootPath, 'templates', 'package.jst')
    )
    ...
    const readme = Fs.readFileSync(
      Path.join(rootPath, 'templates', 'readme.jst')
    )
    const readmeTpl = Dot.template(readme)
    Fs.writeFileSync(
      Path.join(currentRoot, 'README.md'),
      readmeTpl({
    ...
    Fs.writeFileSync(
      Path.join(currentRoot, 'test', 'index.spec.js'),
      testsuiteTpl({
        name: 'hemera-' + args.name,
        topic: args.name
      })
    ...
    Fs.copyFileSync(
      Path.join(rootPath, 'templates', 'configs', '.editorconfig'),
      Path.join(currentRoot, '.editorconfig')
    )

    Fs.copyFileSync(
      Path.join(rootPath, 'templates', 'configs', '.eslintignore'),
      Path.join(currentRoot, '.eslintignore')
    )

    Fs.copyFileSync(
      Path.join(rootPath, 'templates', 'configs', '.eslintrc.json'),
      Path.join(currentRoot, '.eslintrc.json')
    )

    Fs.copyFileSync(
      Path.join(rootPath, 'templates', 'configs', '.prettierignore'),
      Path.join(currentRoot, '.prettierignore')
    )

    Fs.copyFileSync(
      Path.join(rootPath, 'templates', 'configs', '.prettierrc'),
      Path.join(currentRoot, '.prettierrc')
    )
...
vorpal
  .command('clean', 'Clear the console')
  .alias('cls')
  .action(function(args, cb) {
...
vorpal
  .command('services', 'List all available services of your network')
  .validate(function(args) {
    if (hemera) {
      return true
    } else {
...
vorpal
  .command('actions', 'List all available actions of your network')
  .validate(function(args) {
    if (hemera) {
      return true
    } else {
